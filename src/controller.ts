...
import CompletedUser from './models/completedUser';
...
class Controller {
  ...
  resetRoom(roomName: string): void {
    ...
    room.isInGame = false;
    room.completedUsers = [];
    room.users = Controller.resetUsers(room.users);
  }

  private static resetUsers(users: UserInfo[]): UserInfo[] {
...
const storage: Controller = new Controller();
